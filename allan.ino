...
void loop() {
  if (button.isPressed()){
    ...
    while (!button.isPressed() || timer > 3000){  
      delay(10);
      timer+=10;
    }
    ...
    while (1){
      ...
      delay(500); //wait for balls to settle
      colour = get_colour();  //Read colour
      if (colour = 0){  //Copper or unknown
        //move, wait, return
        //resetCounter = 0;
        sortServ.write(60);
        delay(200);
        sortServ.write(90);
      }else if (colour = 2){  //Silver
        //move, wait, return
        //resetCounter = 0;
        sortServ.write(120);
        delay(200);
